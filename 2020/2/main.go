...
func part2() {
	...
	for _, str := range inputs {
		...
		maxMinString := strings.Split(splitStr[0], "-")
		reqLetter := string(splitStr[1][0])
		low, err := strconv.Atoi(maxMinString[0])
		if err != nil {
			panic(err)
		}
		high, err := strconv.Atoi(maxMinString[1])
		if err != nil {
			panic(err)
		}
		password := splitStr[2]
		first, second := " ", " "
		first = string(password[low-1])
		second = string(password[high-1])
		if first != second && (first == reqLetter ||  second == reqLetter) {
			validPasswords = append(validPasswords, str)
		}
	}
	fmt.Printf("# Valid Paswords: %d\n", len(validPasswords))
}
...
func part1() {
	...
	for _, str := range inputs {
		splitStr := strings.Split(str, " ")
		maxMinString := strings.Split(splitStr[0], "-")
		reqLetter := splitStr[1][0]
		letterMap := letterCountMap(splitStr[2])
		low, err := strconv.Atoi(maxMinString[0])
		if err != nil {
			panic(err)
		}
		...
		if letterMap[rune(reqLetter)] >= low && letterMap[rune(reqLetter)] <= high {
			validPasswords = append(validPasswords, str)
		}
	}
	fmt.Printf("# Valid Paswords: %d\n", len(validPasswords))
...
func letterCountMap(str string) map[rune]int {
