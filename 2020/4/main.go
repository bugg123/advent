...
func main() {
	...
	v := validator.New()
	err := v.RegisterValidation("hgt", validateHGT)
	if err != nil {
		panic(err)
	}
	...
	for s.Scan() {
		...
		spaceSplit := strings.Split(strings.TrimSpace(s.Text()), " ")
		for _, str := range spaceSplit {
			passport.addValues(str)
		}
	}
	fmt.Println(len(passportList))
...
type Passport struct {
	...
	Pid string `validate:"required,len=9,numeric"`
	Eyr int    `validate:"required,gte=2020,lte=2030"`
	Ecl string `validate:"required,oneof=amb blu brn gry grn hzl oth"`
	Hgt string `validate:"required,hgt"`
	Hcl string `validate:"required,iscolor"`
	Cid string
}

func (p *Passport) addValues(in string) {
	kv := strings.Split(in, ":")
	if len(kv) != 2 {
		panic(fmt.Sprintf("Can't print %v", kv))
	}
	switch kv[0] {
	case "byr":
...
func validateHGT(fl validator.FieldLevel) bool {
	hgt := fl.Field().String()
...
const testInput string = `ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
const input string = `byr:1983 iyr:2017
