...
func (p *Pos) String() string {
	return fmt.Sprintf("X: %d, Y: %d, Danger: %d", p.x, p.y, p.danger)
}
...
func init() {
	positions = make([][]Pos, 0, len(input)*2)
	for _, str := range input {
		startStop := strings.Split(str, "->")
		coordSlice := make([]Pos, 0, 2)
		for _, coords := range startStop {
			coordSlice = append(coordSlice, convertCommaStringToPos(coords))
		}
		positions = append(positions, coordSlice)
	}
}

func getLine(a, b Pos) []Pos {
	res := make([]Pos, 0)
	if a.x == b.x {
...
func convertCommaStringToPos(s string) Pos {
	...
	if len(coords) != 2 {
		panic("Unable to parse coordinates")
	}
	xCoord, xErr := strconv.Atoi(strings.TrimSpace(coords[0]))
	yCoord, yErr := strconv.Atoi(strings.TrimSpace(coords[1]))
	if yErr != nil || xErr != nil {
		fmt.Printf("X: %q, Y: %q", coords[0], coords[1])
		panic("Failed to translate x or y")
	}
	return Pos{
		x: xCoord,
		y: yCoord,
