...
func main() {
	input := inputs
	finalVal := 0
	for _, uniqFour := range input {
		leftRight := strings.Split(uniqFour, "|")
		nums := determineNums(leftRight[0])
		finalVal += getValue(nums, leftRight[1])
	}
	fmt.Printf("Final Val: %d\n", finalVal)
}

func getValue(nums map[int]map[rune]struct{}, input string) int{
	totalVal := 0
	for _, str := range strings.Split(strings.TrimSpace(input), " ") {
		val, err := getValueOfString(nums, str)
		if err != nil {
			panic(err)
		}
		totalVal += val
...
func getValueOfString(nums map[int]map[rune]struct{}, input string) (int, error){
	for val, mapRune := range nums {
		if getDiff(mapRune, stringToRuneMap(input)) == 0{
			return val, nil
		}
	}
...
func determineNums(input string) map[int]map[rune]struct{} {
	res := make(map[int]map[rune]struct{})
	...
	for _, str := range split {
		switch len(str) {
		case 2:
			res[1] = stringToRuneMap(str)
		case 3:
			res[7] = stringToRuneMap(str)
		case 4:
			res[4] = stringToRuneMap(str)
		case 7:
			res[8] = stringToRuneMap(str)
		default:
			unknown = append(unknown, str)
		}
	}
	// Known: 1,4,7,8
	unknown = findThree(unknown, res)
	// Known: 1,3,4,7,8
	unknown = findNine(unknown, res)
	// Known: 1,3,4,7,8,9
	unknown = findFive(unknown, res)
	// Known: 1,3,4,5,7,8,9
	unknown = findTwo(unknown, res)
	// Known: 1,2,3,4,5,7,8,9
	unknown = findZero(unknown, res)
	// Known: 0,1,2,3,4,5,7,8,9
	res[6] = stringToRuneMap(unknown[0])
	return res
}

func findZero(unknown []string, curNum map[int]map[rune]struct{}) []string {
	// Zero has all of 1 values
	stillUnknown := make([]string, 0)
	oneMap := curNum[1]
	for _, str := range unknown {
		runeMap := stringToRuneMap(str)
		found := true
		for r := range oneMap {
		...
		if found {
			...
			stillUnknown = append(stillUnknown, str)
		}
...
func findTwo(unknown []string, curNum map[int]map[rune]struct{}) []string {
	stillUnknown := make([]string, 0)
	for _, str := range unknown {
		if len(str) == 5 {
			curNum[2] = stringToRuneMap(str)
		} else {
			stillUnknown = append(stillUnknown, str)
		}
	}
...
func findThree(unknown []string, curNum map[int]map[rune]struct{}) []string {
	//Three is length 5 and has all 1 values
	stillUnknown := make([]string, 0)
	for _, str := range unknown {
		oneMap := curNum[1]
		if len(str) == 5 {
...
func findNine(unknown []string, curNum map[int]map[rune]struct{}) []string {
	// Nine is len 6 and diff from 3 by one
	stillUnknown := make([]string, 0, len(unknown))
	for _, str := range unknown {
		if len(str) == 6 {
			runeMap := stringToRuneMap(str)
			diff := getDiff(curNum[3], runeMap)
			if diff == 1 {
				curNum[9] = runeMap
			} else {
				stillUnknown = append(stillUnknown, str)
...
func findFive(unknown []string, curNum map[int]map[rune]struct{}) []string {
	// Five is len 5 and diff one from 9
	stillUnknown := make([]string, 0, len(unknown))
	for _, str := range unknown {
		if len(str) == 5 {
			runeMap := stringToRuneMap(str)
			diff := getDiff(curNum[9], runeMap)
			if diff == 1 {
				curNum[5] = runeMap
			} else {
				stillUnknown = append(stillUnknown, str)
...
func getDiff(a, b map[rune]struct{}) int {
	res := 0
	if len(b) > len(a) {
		a, b = b, a
	}
...
func stringToRuneMap(in string) map[rune]struct{} {
